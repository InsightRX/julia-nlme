...
- **FOCE vs FOCE-I**: pass `interaction=true` to `fit()` to enable eta-epsilon interaction (FOCE-I). The only difference is that `R` (residual variance matrix) is evaluated at `ipred_hat` (individual prediction) instead of `f0` (population prediction). Has no effect for additive-only error models. Default is `false` (standard FOCE).
